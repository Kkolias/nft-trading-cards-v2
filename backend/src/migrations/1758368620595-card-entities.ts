...
export class CardEntities1758368620595 implements MigrationInterface {
    name = 'CardEntities1758368620595'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE "packs" ("id" uuid NOT NULL DEFAULT uuid_generate_v4(), "createdAt" TIMESTAMP NOT NULL DEFAULT now(), "updatedAt" TIMESTAMP NOT NULL DEFAULT now(), "name" character varying NOT NULL, "description" character varying NOT NULL, "imageUrl" character varying NOT NULL, "price" numeric(10,2) NOT NULL, "configJson" jsonb NOT NULL, CONSTRAINT "PK_da3c6e998aaa9331767c51e7f91" PRIMARY KEY ("id"))`);
        await queryRunner.query(`CREATE TYPE "public"."cards_rarity_enum" AS ENUM('common', 'rare', 'epic', 'legendary')`);
        await queryRunner.query(`CREATE TABLE "cards" ("id" uuid NOT NULL DEFAULT uuid_generate_v4(), "createdAt" TIMESTAMP NOT NULL DEFAULT now(), "updatedAt" TIMESTAMP NOT NULL DEFAULT now(), "name" character varying NOT NULL, "rarity" "public"."cards_rarity_enum" NOT NULL DEFAULT 'common', "imageUrl" character varying NOT NULL, "pack_id" uuid, CONSTRAINT "PK_5f3269634705fdff4a9935860fc" PRIMARY KEY ("id"))`);
        await queryRunner.query(`CREATE TABLE "minted_cards" ("id" uuid NOT NULL DEFAULT uuid_generate_v4(), "createdAt" TIMESTAMP NOT NULL DEFAULT now(), "updatedAt" TIMESTAMP NOT NULL DEFAULT now(), "tokenId" character varying NOT NULL, "ownerWallet" character varying NOT NULL, "mintedAt" TIMESTAMP WITH TIME ZONE NOT NULL, "card_id" uuid, CONSTRAINT "REL_a9d78d27ecbaec0b23fb43c5d4" UNIQUE ("card_id"), CONSTRAINT "PK_088b3f1dc7e42a22988a18255cd" PRIMARY KEY ("id"))`);
        await queryRunner.query(`CREATE TYPE "public"."trades_status_enum" AS ENUM('open', 'filled', 'canceled')`);
        await queryRunner.query(`CREATE TABLE "trades" ("id" uuid NOT NULL DEFAULT uuid_generate_v4(), "createdAt" TIMESTAMP NOT NULL DEFAULT now(), "updatedAt" TIMESTAMP NOT NULL DEFAULT now(), "tokenId" character varying NOT NULL, "sellerWallet" character varying NOT NULL, "price" numeric(10,2) NOT NULL, "status" "public"."trades_status_enum" NOT NULL DEFAULT 'open', CONSTRAINT "PK_c6d7c36a837411ba5194dc58595" PRIMARY KEY ("id"))`);
